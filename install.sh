...
mkdir -p "$HOME/.ssh/"
mv /u16java/config "$HOME/.ssh/"
mv /u16java/90forceyes /etc/apt/apt.conf.d/
...
echo 'M2_HOME=/usr/local/apache-maven-$APACHE_MAVEN/' >> /etc/drydock/.env
...
for file in /u16java/version/*.sh;
do
  $file
done
